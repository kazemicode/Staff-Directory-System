...
if 'last_name' not in form:
    ...
    print("<body><p>You must enter a last name. <a href=\"/staff.html\">Return to search</a> </p></body></html>")
    sys.exit()
else:
...
dept_query = 'SELECT last_name, first_name, period, course_title, room_number\
                    ...
                    JOIN courses c\
                        ON sch.course_id = c.course_id\
                    WHERE (department_id = %s OR department_id = 11)'
...
if first_name != None and last_name != None and department != 0:
    ...
    if rows is None:
        print('<p>No staff members match your query.</p>', last_name, first_name)
    else:
